...
if config["params"]["profiling"]["metaphlan"]["do_v40"] and \
   config["params"]["profiling"]["humann"]["do_v40"]:
    if config["params"]["profiling"]["humann"]["update_config"]:
        rule profiling_humann4_config:
            output:
            ...
            params:
                database_utility_mapping = config["params"]["profiling"]["humann"]["database_utility_mapping_v4"],
                database_nucleotide = config["params"]["profiling"]["humann"]["database_nucleotide_v4"],
                database_protein = config["params"]["profiling"]["humann"]["database_protein_v4"],
                threads = config["params"]["profiling"]["threads"]
            priority:
                20
            shell:
    ...
    #rule profiling_humann4_build_chocophlan_pangenome_db:
    #    input:
    #        profile = os.path.join(
    #            config["output"]["profiling"],
    ...
    #        expand(os.path.join(
    #            config["output"]["profiling"],
    #            "database/humann4/{{sample}}/{{sample}}_bowtie2_index.{suffix}"),
    #               suffix=["1.bt2", "2.bt2", "3.bt2", "4.bt2", "rev.1.bt2", "rev.2.bt2"])
    #    log:
    #        os.path.join(config["output"]["profiling"],
    #                     "logs/humann4/{sample}.humann4.build_pandb.log")
    #    params:
    #        basename = "{sample}",
    #        wrapper_dir = WRAPPER_DIR,
    #        db_dir = os.path.join(config["output"]["profiling"], "database/humann4/{sample}"),
    #        prescreen_threshold = config["params"]["profiling"]["humann"]["prescreen_threshold"]
    #    shell:
    #        '''
    ...
    rule profiling_humann4:
        input:
            tag = os.path.join(config["output"]["profiling"], ".humann4.config.done"),
            reads = profiling_input_with_short_reads,
            profile = os.path.join(
                config["output"]["profiling"],
                "profile/metaphlan40/{sample}/{sample}.metaphlan4.abundance.profile.tsv")
        output:
            genefamilies = os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{sample}/{sample}_genefamilies.tsv"),
            pathabundance = os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{sample}/{sample}_pathabundance.tsv"),
            pathcoverage = os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{sample}/{sample}_pathcoverage.tsv")
        log:
            os.path.join(config["output"]["profiling"], "logs/humann4/{sample}.humann4.log")
        benchmark:
            os.path.join(config["output"]["profiling"],
                         "benchmark/humann4/{sample}.humann4.benchmark.txt")
        priority:
            20
        conda:
            config["envs"]["biobakery40"]
        params:
            ...
            index = os.path.join(config["output"]["profiling"],
                                 "database/humann4/{sample}/{sample}_bowtie2_index"),
    ...
    rule profiling_humann4_postprocess:
        input:
            expand(os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{{sample}}/{{sample}}_{target}.tsv"),
                   target=["genefamilies", "pathabundance", "pathcoverage"])
        output:
            targets = expand(os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{{sample}}/{{sample}}_{target}_{norm}.tsv"),
                target=["genefamilies", "pathabundance", "pathcoverage"],
                norm=config["params"]["profiling"]["humann"]["normalize_method"]),
            groupprofiles = expand(os.path.join(
                config["output"]["profiling"],
                "profile/humann4/{{sample}}/{{sample}}_{group}_groupped.tsv"),
                group=config["params"]["profiling"]["humann"]["map_database"])
        log:
            os.path.join(config["output"]["profiling"],
    ...
    rule profiling_humann4_join:
        ...
        output:
            targets = expand(
                os.path.join(
                    config["output"]["profiling"],
                    "report/humann4/humann4_{target}_joined.tsv"),
                target=["genefamilies", "pathabundance", "pathcoverage"]),
            targets_norm = expand(
               os.path.join(
                   config["output"]["profiling"],
                   "report/humann4/humann4_{target}_{norm}_joined.tsv"),
                target=["genefamilies", "pathabundance", "pathcoverage"],
                norm=config["params"]["profiling"]["humann"]["normalize_method"]),
            groupprofile = expand(
                os.path.join(
                    config["output"]["profiling"],
                    "report/humann4/humann4.{group}.joined.tsv"),
                group=config["params"]["profiling"]["humann"]["map_database"])
        log:
            os.path.join(config["output"]["profiling"],
                         "logs/humann4/humann4_join.log")
        conda:
            config["envs"]["biobakery4"]
        params:
            wrapper_dir =WRAPPER_DIR,
            input_dir = os.path.join(config["output"]["profiling"], "profile/humann4"),
            normalize_method = config["params"]["profiling"]["humann"]["normalize_method"],
            map_database = config["params"]["profiling"]["humann"]["map_database"]
        priority:
            20
    ...
    rule profiling_humann4_split_stratified:
        input:
            targets = expand(
                os.path.join(
                    config["output"]["profiling"],
        ...
        output:
            expand([
                os.path.join(
                    config["output"]["profiling"],
                    "report/humann4/humann4_{target}_joined_{suffix}.tsv"),
                os.path.join(
                    config["output"]["profiling"],
                    "report/humann4/humann4_{target}_{norm}_joined_{suffix}.tsv"),
                os.path.join(
                    config["output"]["profiling"],
                    "report/humann4/humann4.{group}.joined_{suffix}.tsv")],
                   target=["genefamilies", "pathabundance", "pathcoverage"],
                   norm = config["params"]["profiling"]["humann"]["normalize_method"],
                   group=config["params"]["profiling"]["humann"]["map_database"],
                   suffix=["stratified", "unstratified"])
        log:
            os.path.join(config["output"]["profiling"],
                         "logs/humann4/humann4_split_stratified.log")
        conda:
            config["envs"]["biobakery4"]
        params:
            wrapper_dir = WRAPPER_DIR,
            output_dir = os.path.join(config["output"]["profiling"], "report/humann4"),
            map_database = config["params"]["profiling"]["humann"]["map_database"]
        priority:
            20
        shell:
