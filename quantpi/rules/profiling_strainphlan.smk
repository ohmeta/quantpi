...
rule profiling_strainphlan42_sample2markers:
    input:
        database_pkl = expand(os.path.join(
            config["params"]["profiling"]["metaphlan"]["bowtie2db_v42"], "{index}.pkl"),
            index = config["params"]["profiling"]["metaphlan"]["index_prefix_v42"]),
        sam = os.path.join(
            config["output"]["profiling"],
            "profile/metaphlan42/{sample}/{sample}.sam.bz2"),
        aln = os.path.join(
            config["output"]["profiling"],
            "profile/metaphlan42/{sample}/{sample}.bowtie2.bz2")
    output:
        os.path.join(
            config["output"]["profiling"],
            "profile/strainphlan42/consensus_markers/{sample}.pkl")
    log:
        os.path.join(
            config["output"]["profiling"],
            "logs/strainphlan42_sample2markers/{sample}.strainphlan42_sample2markers.log")
    benchmark:
        os.path.join(
            config["output"]["profiling"],
            "benchmark/strainphlan42_sample2markers/{sample}.strainphlan42_sample2markers.benchmark.txt")
    params:
        outdir =  os.path.join(config["output"]["profiling"], "profile/strainphlan42/consensus_markers")
    conda:
...
if config["params"]["profiling"]["strainphlan"]["reference_genome"]["use"]:
    ...
    STRAINPHLAN_CLADES_V42 = \
        pd.read_csv(config["params"]["profiling"]["strainphlan"]["clades_tsv_v42"], sep="\t")\
        .set_index("clade")
    STRAINPHLAN_CLADES_LIST_V42 = STRAINPHLAN_CLADES_V42.index.unique()


    rule profiling_strainphlan42_prepare_reference_genome:
        input:
            reference_genome = lambda wildcards: STRAINPHLAN_CLADES_V42.loc[wildcards.clade, "fna_path"]
        output:
            reference_genome = os.path.join(
                config["output"]["profiling"],
                "databases/strainphlan42/reference_genomes/{clade}.fna")
        threads:
            1
        run:
            if input.reference_genome.endswith(".gz"):
    ...
    checkpoint profiling_strainphlan42_print_clades:
        input:
            database_pkl = expand(os.path.join(
                config["params"]["profiling"]["metaphlan"]["bowtie2db_v42"], "{index}.pkl"),
                index = config["params"]["profiling"]["metaphlan"]["index_prefix_v42"]),
            consensus_markers = expand(os.path.join(
                ...
                sample=SAMPLES_ID_LIST)
        output:
            markers_txt = os.path.join(config["output"]["profiling"], "databases/strainphlan42/clade_markers.txt"),
            markers_txt_gt = expand(
                os.path.join(config["output"]["profiling"], "databases/strainphlan42/clade_markers.txt.gt{num}"),
                num=config["params"]["profiling"]["strainphlan"]["clade_detected_in_min_samples_num"]),
            markers_dir = directory(os.path.join(config["output"]["profiling"], "databases/strainphlan42/clade_markers"))
        log:
            os.path.join(config["output"]["profiling"], "logs/strainphlan42_print_clades/strainphlan42_print_clades.log")
        benchmark:
            os.path.join(config["output"]["profiling"], "benchmark/strainphlan42_print_clades.txt")
        params:
            marker_in_n_samples = config["params"]["profiling"]["strainphlan"]["marker_in_n_samples"],
            clade_detected_in_min_samples_num = config["params"]["profiling"]["strainphlan"]["clade_detected_in_min_samples_num"]
        conda:
    ...
    def aggregate_profiling_strainphlan42_extract_markers(wildcards):
        checkpoint_output = checkpoints.profiling_strainphlan42_print_clades.get(**wildcards).output.markers_dir
        ...
        clades_done = expand(os.path.join(
            config["output"]["profiling"],
            "profile/strainphlan42/clade_markers/{clade}/done"),
            clade = clades)

        return clades_done


rule profiling_strainphlan42_extract_markers:
    input:
        database_pkl = expand(os.path.join(
            config["params"]["profiling"]["metaphlan"]["bowtie2db_v42"], "{index}.pkl"),
            index = config["params"]["profiling"]["metaphlan"]["index_prefix_v42"])
    output:
        clade_marker = os.path.join(
            config["output"]["profiling"],
            "databases/strainphlan42/clade_markers/{clade}/{clade}.fna")
    log:
        os.path.join(
            config["output"]["profiling"],
            "logs/strainphlan42_extract_markers/{clade}.strainphlan42_extract_markers.log")
    benchmark:
        os.path.join(
            config["output"]["profiling"],
            "benchmark/strainphlan42_extract_markers/{clade}.strainphlan42_extract_markers.benchmark.txt")
    conda:
        config["envs"]["biobakery42"]
    params:
        clade = "{clade}",
        outdir = os.path.join(config["output"]["profiling"], "databases/strainphlan42/clade_markers/{clade}")
    priority:
        20
    threads:
        1
...
rule profiling_strainphlan42:
    input:
        ...
        consensus_markers = expand(os.path.join(
            ...
            sample=SAMPLES_ID_LIST)
    output:
        done = os.path.join(config["output"]["profiling"], "profile/strainphlan42/clade_markers/{clade}/done")
    log:
        os.path.join(
            config["output"]["profiling"],
            "logs/strainphlan42/{clade}.strainphlan42.log")
    benchmark:
        os.path.join(
            config["output"]["profiling"],
            "benchmark/strainphlan42/{clade}.strainphlan42.benchmark.txt")
    conda:
        config["envs"]["biobakery42"]
    params:
        clade = "{clade}",
        outdir = os.path.join(config["output"]["profiling"], "profile/strainphlan42/clade_markers/{clade}"),
        marker_in_n_samples = config["params"]["profiling"]["strainphlan"]["marker_in_n_samples"],
        sample_with_n_markers = config["params"]["profiling"]["strainphlan"]["sample_with_n_markers"],
        breadth_thres = config["params"]["profiling"]["strainphlan"]["breadth_thres"],
        phylophlan_mode = config["params"]["profiling"]["strainphlan"]["phylophlan_mode"],
        reference_opts = "--references %s" % \
        os.path.join(config["output"]["profiling"], "databases/strainphlan42/reference_genomes/{clade}.fna") \
        if config["params"]["profiling"]["strainphlan"]["reference_genome"]["use"] \
        else "",
        opts = config["params"]["profiling"]["strainphlan"]["external_opts_v42"]
    priority:
        20
    threads:
        config["params"]["profiling"]["threads"]
...
if config["params"]["profiling"]["strainphlan"]["do_v42"]:
    if config["params"]["profiling"]["strainphlan"]["reference_genome"]["use"]:
        rule profiling_strainphlan42_all:
            input:
                expand(os.path.join(
                    config["output"]["profiling"],
                    "profile/strainphlan42/clade_markers/{clade}/done"),
        ...
        rule profiling_strainphlan42_all:
            input:
                aggregate_profiling_strainphlan42_extract_markers
else:
    rule profiling_strainphlan42_all:
        input: