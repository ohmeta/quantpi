rule profiling_all:
    input:
        rules.profiling_kraken2_all.input,
        rules.profiling_kraken2_bracken_all.input,
        rules.profiling_krakenuniq_all.input,
        rules.profiling_krakenuniq_bracken_all.input,
        rules.profiling_metaphlan2_all.input,
        rules.profiling_metaphlan3_all.input,
        rules.profiling_metaphlan40_all.input,
        rules.profiling_metaphlan41_all.input,
        rules.profiling_metaphlan42_all.input,
        rules.profiling_strainphlan3_all.input,
        rules.profiling_strainphlan40_all.input,
        rules.profiling_strainphlan41_all.input,
        rules.profiling_strainphlan42_all.input,
        rules.profiling_kmcp_all.input,
        rules.profiling_genomecov_all.input,
        rules.profiling_genome_coverm_all.input,
        rules.profiling_humann2_all.input,
        rules.profiling_humann35_all.input,
        rules.profiling_humann38_all.input,
        rules.profiling_humann39_all.input


localrules:
    profiling_kraken2_all,
    profiling_kraken2_bracken_all,
    profiling_krakenuniq_all,
    profiling_krakenuniq_bracken_all,
    profiling_metaphlan2_all,
    profiling_metaphlan3_all,
    profiling_metaphlan40_all,
    profiling_metaphlan41_all,
    profiling_metaphlan42_all,
    profiling_strainphlan3_all,
    profiling_strainphlan40_all,
    profiling_strainphlan41_all,
    profiling_strainphlan42_all,
    profiling_kmcp_all,
    profiling_genomecov_all,
    profiling_genome_coverm_all,
    profiling_humann2_all,
    profiling_humann35_all,
    profiling_humann38_all,
    profiling_humann39_all,
    profiling_all